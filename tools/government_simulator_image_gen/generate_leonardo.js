#!/usr/bin/env node
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// 政府シミュレーター 画像一括生成ツール（Leonardo.ai版）
// leonardo-ai-image-gen スキルのパターンをそのまま流用。
//
// 使い方（PowerShellで事前に $env:LEONARDO_API_KEY="xxxx" を実行しておく）:
//   node generate_leonardo.js --count 2       # 先頭2件だけ（未生成分のみ、品質確認用）
//   node generate_leonardo.js --ids app_icon,minister_finance
//   node generate_leonardo.js --all           # 残り全44枚をまとめて生成（推奨）
//   node generate_leonardo.js --all --force   # 既存分も含めて全部作り直す
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

const fs = require('fs');
const path = require('path');
const { buildAssetList, buildPrompt } = require('./prompt_builder');
...
async function main() {
  ...
  console.log(`📋 アセット定義を ${allAssets.length} 件読み込みました`);

  let target;
  if (opts.ids) {
    target = allAssets.filter((a) => opts.ids.includes(a.id));
  } else if (opts.all) {
    target = allAssets;
  } else {
    const n = opts.count || 2;
    target = allAssets.slice(0, n);
  }
  ...
  for (const asset of toGenerate) {
    const { prompt, negativePrompt } = buildPrompt(asset);
    process.stdout.write(`  ${asset.id} (${asset.nameJp} / ${asset.category}) ... `);
    try {
